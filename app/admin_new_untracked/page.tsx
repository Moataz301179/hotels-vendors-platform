...
import { fmtDateTime } from "@/lib/stubs-export";
import AppShell, { Guard, RequireAuth } from "@/lib/stubs-export";
import { Card, PageHead, Stat, StatePill, btnCls } from "@/lib/stubs-export";
import { IcAlert, IcArrow, IcBuilding, IcHistory, IcScale, IcTruck, IcUsers } from "@/lib/stubs-export";

export default function AdminPage() {
  const { t, lang } = usePrefs();
  const { data } = useApp();

  const flags = data.deliveries.filter((d: { delayed: boolean; status: string }) => d.delayed && d.status !== "delivered");
  const openTx = data.orders.filter((o: any) => o.approval.state !== "rejected" && o.receipt !== "complete").length;

  const stageRows = [
    { l: t("state.pending"), n: data.orders.filter((o: any) => o.approval.state === "pending").length },
    { l: t("state.acknowledged"), n: data.orders.filter((o: any) => o.fulfillment === "acknowledged").length },
    { l: t("state.preparing"), n: data.orders.filter((o: any) => o.fulfillment === "preparing").length },
    { l: t("state.in_transit"), n: data.orders.filter((o: any) => ["shipped", "in_transit", "out_for_delivery"].includes(o.fulfillment)).length },
    { l: t("state.delivered"), n: data.orders.filter((o: any) => o.fulfillment === "delivered" && o.receipt !== "complete").length },
    { l: t("state.complete"), n: data.orders.filter((o: any) => o.receipt === "complete").length },
  ];
  const maxStage = Math.max(1, ...stageRows.map((x) => x.n));
  const governed = data.orders.filter((o: any) => o.approval.state !== "rejected");
      ...
      <AppShell active="/admin">
        <Guard roles={["platform_admin"]}>
          ...
          <div className="grid grid-air sm:grid-cols-2 xl:grid-cols-4">
            <Stat label={t("admin.kpiTenants")} value={data.tenants.filter((x: any) => x.status === "active").length} tone="ok" />
